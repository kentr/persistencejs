 ...
 * @param iOSLocation
 * @param iOSDatabaseLocation
 */
persistence.store.cordovasql.config = function (persistence, dbname, dbversion, description, size, backgroundProcessing, iOSLocation, iOSDatabaseLocation) {
  var conn = null;
   ...
   * @returns {{}}
   */
  persistence.db.sqliteplugin.connect = function (dbname, backgroundProcessing, iOSLocation, iOSDatabaseLocation) {
    var that = {};
    var conn = window.sqlitePlugin.openDatabase({name: dbname, bgType: backgroundProcessing, location: (iOSLocation || 0), iosDatabaseLocation: iOSDatabaseLocation});
   ...
   * @returns {*}
   */
  persistence.db.connect = function (dbname, dbversion, description, size, backgroundProcessing, iOSLocation, iOSDatabaseLocation) {
    if (persistence.db.implementation == "sqliteplugin") {
      return persistence.db.sqliteplugin.connect(dbname, backgroundProcessing, iOSLocation, iOSDatabaseLocation);
  ...
  // Make the connection
  conn = persistence.db.connect(dbname, dbversion, description, size, backgroundProcessing, iOSLocation, iOSDatabaseLocation);
  if (!conn) {
